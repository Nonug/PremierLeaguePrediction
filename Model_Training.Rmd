...
## Ensemble Model
```{r}
algorithmList <- c("fv", "adaboost", "earth", "xgbDART", "svmRadial")
...
compute.auc <- function(model, test.data) {
    ...
    model.roc <- roc(
        predictor = test_df,
        response = test.data$FTR,
        levels = rev(levels(test.data$FTR))
    )
...
```{r}
multinom.roc <- compute.auc(multinom_pred, test_df)
ggsave(multinom.roc,
    filename = "./img/multinom_test_res.png",
    height = 6, width = 6
)
...
model.roc <- multiclass.roc(
    as.numeric(test_df$FTR),
    as.numeric(multinom_pred)
)
```