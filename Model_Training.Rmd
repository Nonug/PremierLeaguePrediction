...
p_load(
    ...
    BiocManager, doMC,
    DataExplorer, nnet, broom, glmnet, MLmetrics,
...
## Import dataset
```{r}
train_df_timestamped <- read.csv("./data/training_data_fwd.csv")

train_df <- subset(train_df_timestamped,
    select = -c(Season, Date, Date_interval)
)

test_df <- read.csv("./data/testing_data_fwd.csv")
...
xgbdart_mod
fitted_xgbdart <- predict(xgbdart_mod)
xgbdart_mod_plot <- plot(xgbdart_mod,
    main = "Model Accuracies with caret + xgbTree"
)
lattice_save(xgbdart_mod_plot, "Model Accuracies with xgbTree")

sink(file = "./outputs/confmat_xgbtree.txt")
confusionMatrix(reference = y, data = fitted_xgbdart, mode = "everything")
sink(file = NULL)

# Feature importance
xgbdart_imp <- varImp(xgbdart_mod, scale = FALSE)
xgbdart_imp_plot <- ggplot(xgbdart_imp, top = 20, aes(fill = feature)) +
    ggtitle("Feature importance ranked (top 20) in xgbTree model")

ggsave(xgbdart_imp_plot,
...
## Ensemble Model
```{r}
algorithmList <- c("glm", "xgbDART", "svmRadial")

set.seed(100)
models <- caretList(FTR ~ ., data = train_df, trControl = trainControl, methodList = algorithmList)
...
SVM_pred <- predict(svmradial, test_df)
XgbLinear_pred <- predict(xgboost_mod, test_df)
xgbTree_pred <- predict(xgbdart_mod, test_df)

# confusionMatrix(reference = test_df$FTR, data = multinom_pred)

multinom_test_res <- cbind(
    test_df$FTR,
    predict(multinom_CV, newdata = test_df, type = "prob"), multinom_pred
)
colnames(multinom_test_res) <- c("truth", "D", "A", "H", "pred")
# multiClassSummary(multinom_test_res, lev = levels(train_df$FTR), model = multinom_CV)
metrics(multinom_test_res, truth, pred)

res_plot <- plot(roc_auc(multinom_test_res, truth, D:H))
ggsave(res_plot,
    filename = "./img/multinom_test_res.png",
    height = 6, width = 6
)
```
...
```{r}
compute.auc <- function(model, test.data) {
    # model.probs <- predict(model, test.data[, 1:34], type = "prob")
    # multinom_prob <- predict(multinom_CV, newdata = test_df, type = "prob")
    model.probs <- predict(model, newdata = test.data, type = "prob")
    ...
    # model.roc.fp <- 1 - model.roc$specificities
    # model.roc.tp <- model.roc$sensitivities
    # model.roc.plot <- plot(
    #     x = model.roc.fp, y = model.roc.tp, type = "l",
    #     xlab = "false positive rate (1 - specificity)",
    #     ylab = "true positive rate (sensitivity)",
    #     main = "ROC Curve"
    # )

    # model.auc <- model.roc$auc
...
```{r}
multinom.roc <- compute.auc(multinom_CV, test_df)
